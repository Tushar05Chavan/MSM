                          ...
                          GestureDetector(
                            ...
                            child: Container(
                              ...
                              decoration: BoxDecoration(
                                ...
                                border: Border.all(
                                    width: 1.5, color: const Color(0xff203c92)),
                              ),
                              child: Center(
                                  child: Text(
              ...
              GetBuilder<ForDashboardViewModel>(
                builder: (controller) {
                  if (controller.apiResponse.status == Status.COMPLETE) {
                    ...
                    return Column(
                        children:
                            List.generate(forDashboardData.length, (index) {
                      return Container(
                        width: Get.width,
                        height: 60,
          ...
          child: Column(
            children: [
              ...
              GetBuilder<RecentApplicationsNewEnhanceViewModel>(
                builder: (controller) {
                  if (controller.apiResponse.status == Status.COMPLETE) {
                    List<RecentApplicationsNewEnhanceResponseModel> recentData =
                        controller.apiResponse.data;
                    return Column(
        ...
        GetBuilder<NewsViewModel>(
          builder: (controller) {
            if (controller.apiResponse.status == Status.COMPLETE) {
              ...
              List<ns.NewsResponseModel> newsData = controller.apiResponse.data;
              if (newsData.isEmpty) {
                print('no dattatata');
                ...
                children: List.generate(newsData.length, (index) {
                  return Container(
                    margin: EdgeInsets.symmetric(
                        horizontal: Get.width * 0.03, vertical: 9),
                    padding: EdgeInsets.all(Get.width * 0.03),
                    decoration: BoxDecoration(
                      borderRadius: BorderRadius.circular(10),
                      color: Colors.white,
